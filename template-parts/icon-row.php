<div class="no-gutters row">
    <div id="box-one" class="col flex-col">
        <a href="<?= SITEURL ?>/other-natural-supplements-el-paso/" class="icon-button">
            <div class="box">
                <img src="<?=IMGURL?>natural-supplements.jpg" alt="Natural Supplements">
            </div>

            <div class="box-bottom"><?php get_template_part('template-parts/icons/icon', '1'); ?>
                <span>NATURAL SUPPLEMENTS</span></div>
        </a>
    </div>

    <div id="box-two" class="col flex-col">
        <a href="<?= SITEURL ?>/products/" class="icon-button">
            <div class="box">
                <img src="<?=IMGURL?>natural-teas.jpg" alt="Natural Teas">
            </div>

            <div class="box-bottom"><?php get_template_part('template-parts/icons/icon', '2'); ?>
                <span>NATURAL TEAS</span></div>
        </a>
    </div>

    <div id="box-three" class="col flex-col">
        <a href="<?= SITEURL ?>/therbal-tea-el-paso/" class="icon-button">
            <div class="box">
                <img src="<?=IMGURL?>natural-herbal-teas.jpg" alt="Natural Herbal Teas">
            </div>
            <div class="box-bottom"><?php get_template_part('template-parts/icons/icon', '3'); ?>
                <span>NATURAL HERBAL TEAS</span></div>
        </a>
    </div>

    <div id="box-three" class="col flex-col">
        <a href="<?= SITEURL ?>/beauty-products-el-paso/" class="icon-button">
            <div class="box">
                <img src="<?=IMGURL?>health-beauty.jpg" alt="Health & Beauty">
            </div>
            <div class="box-bottom"><?php get_template_part('template-parts/icons/icon', '4'); ?>
                <span>HEALTH & BEAUTY</span></div>
        </a>
    </div>

    <div id="box-three" class="col flex-col">
        <a href="<?= SITEURL ?>/over-the-counter-medical-items-el-paso/" class="icon-button">
            <div class="box">
                <img src="<?=IMGURL?>otc-first-aid.jpg" alt="OTC First Aid">
            </div>
            <div class="box-bottom"><?php get_template_part('template-parts/icons/icon', '5'); ?>
                <span>OTC FIRST AID</span></div>
        </a>
    </div>
</div>